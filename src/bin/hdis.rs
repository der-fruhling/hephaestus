...
use sha2::Digest;
use hephaestus::{BinaryEncodable, MetadataContentKind, Item, LinkageFlags, Module, ModuleFlags, Target, Source, ImportedModule, StructField};
...
fn global_name(name: Option<&str>, idx: usize) -> NameOrIdx<'_, '@'> { NameOrIdx(name, idx) }

fn main() -> Result<(), Box<dyn Error>> {
    let cli = Cli::parse();

    let mut input = File::open(&cli.input_file).unwrap();
    let mut out: BufWriter<Box<dyn Write>> = BufWriter::new(if let Some(output_file) = &cli.output_file {
    ...
    for custom_feature in &module.metadata {
        let disp: Box<dyn Display> = match custom_feature.kind {
            MetadataContentKind::String => Box::new(String::from_utf8(custom_feature.content.clone().into()).unwrap_or_else(|e| {
            ...
            MetadataContentKind::Target => Box::new(Target::from_u16(custom_feature.content.clone().get_u16())),
            MetadataContentKind::DWARF => Box::new("<DWARF debug information>"),
            _other => Box::new(hex::encode(&custom_feature.content))
        };
        writeln!(out, "[!metadata({}, {}, {})]", custom_feature.name, custom_feature.kind, disp)?;
    }
    ...
    for (i, global) in module.all_globals().enumerate() {
        match global {
            Item::Global(g) => {
                if g.name.is_some() { writeln!(out, "# index = @%{i}")?; }
            ...
            Item::Function(f) => {
                if f.name.is_some() { writeln!(out, "# index = @%{i}")?; }
            ...
            Item::ImportedModule(ImportedModule::WitComponent(name, content)) => {
                ...
                let canon_path = cli.output_file
                    ...
                    .canonicalize()?;
                let canon_parent = canon_path
                    .parent().ok_or("no parent could be found")?;
                ...
                write!(out, "import world \"{name}\" from component \"{}\" as @%{i}", path.strip_prefix(canon_parent)?.to_string_lossy())?;
            }
            Item::Struct(s) => {
                if s.name.is_some() { writeln!(out, "# index = @%{i}")?; }
                pretty_print_linkage_as_attributes(&mut out, s.linkage, &s.source)?;
                writeln!(out, "struct {} {{", global_name(s.name.as_deref(), i))?;
                
                for value in &s.values {
                    match value {
                        StructField::Data { layout: _ /* TODO implement layout */, ty, name} => {
                            writeln!(out, "    {name}: {ty},")?;
                        }
            ...
            Item::Enum(e) => {
                if e.name.is_some() { writeln!(out, "# index = @%{i}")?; }
                pretty_print_linkage_as_attributes(&mut out, e.linkage, &e.source)?;
                writeln!(out, "enum {}: {} {{", global_name(e.name.as_deref(), i), e.backing_type)?;
...
fn pretty_print_linkage_as_attributes(out: &mut BufWriter<Box<dyn Write>>, linkage: LinkageFlags, source: &Option<Source>) -> Result<(), Box<dyn Error>> {
    if linkage.contains(LinkageFlags::EXPORTED) {
        write!(out, "export ")?;

        if linkage.contains(LinkageFlags::ACCESSIBLE) { write!(out, "accessible ")?; }
        else { write!(out, "noaccessible ")?; }
        if linkage.contains(LinkageFlags::DOCUMENTED) { write!(out, "document ")?; }
        else { write!(out, "nodocument ")?; }
    }

    if linkage.contains(LinkageFlags::INTERNAL) {
        write!(out, "internal ")?;

        if linkage.contains(LinkageFlags::FORCE_INLINE) { write!(out, "inline(always) ")?; }
        else if linkage.contains(LinkageFlags::INLINE) { write!(out, "inline ")?; }
    }
